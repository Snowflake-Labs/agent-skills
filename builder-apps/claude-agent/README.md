...
A web application that provides a Claude Code agent interface with integrated Snowflake tools. Users interact with Claude through a chat interface, and the agent can execute SQL queries, manage pipelines, and more on their Snowflake account.
...
- **Skills pre-loaded** — skills from [`snowflake-skills/`](../../snowflake-skills/) and [`general-skills/`](../../general-skills/) are automatically available to the agent.
